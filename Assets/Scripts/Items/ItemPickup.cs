...
public class ItemPickup : MonoBehaviour
{
                ...
                case "Item":
                    {
                        if (Input.GetMouseButtonDown(0))
                        {
                            ...
                            if (heldRb != null)
                            {
                                sensetivityLose = (playerStats.strength / heldRb.mass) * sensetivityLoseCoef;
                                if (sensetivityLose > 1) sensetivityLose = 1;
                                playerLook.mouseSensitivity *= sensetivityLose;
                                Debug.Log("sensetivity lose: " + sensetivityLose);
    ...
    void HandlePhysicsForHeldItem()
    {
        ...
        if (heldRb != null)
        {
            ...
            if (isRotating)
            {
                float rotX = Input.GetAxis("Mouse X") * rotationSpeed * sensetivityLose * Time.fixedDeltaTime;
                float rotY = Input.GetAxis("Mouse Y") * rotationSpeed * sensetivityLose * Time.fixedDeltaTime;
